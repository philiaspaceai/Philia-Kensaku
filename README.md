# Philia Kensaku 🌸
> **Final Update: 26 Desember 2025**

![License: CC BY-NC-SA 4.0](https://img.shields.io/badge/License-CC%20BY--NC--SA%204.0-lightgrey.svg)
![Status: Active](https://img.shields.io/badge/Status-Active-success.svg)
![Powered By: Google AI](https://img.shields.io/badge/Powered%20By-Google%20AI%20Studio-blue.svg)

**Platform Pencarian TSK (Registered Support Organizations) & Strategi Karir untuk Tokutei Ginou Indonesia.**

Philia Kensaku adalah inisiatif **Non-Profit** dari komunitas untuk membantu pekerja Indonesia mencari lembaga pendukung (TSK) yang aman, valid, dan terpercaya di Jepang tanpa biaya sepeserpun.

---

## 🌟 Fitur Utama (Update v2.0)

### 1. 🔍 Pencarian Cerdas (Smart Search)
- **Pencarian Cepat**: Cari berdasarkan nama perusahaan, No. Registrasi, atau wilayah.
- **Advanced Filter**: Filter berdasarkan **Prefektur**, **Bahasa Indonesia**, dan **Dukungan Legal vs Opsional**.

### 2. 🕵️ Detektif TSK (AI Powered)
Jangan tertipu broker nakal! Gunakan tombol **"Cari Tahu"** yang ditenagai oleh **Super Prompt AI** (Google Gemini) untuk membongkar:
- Jejak digital & akun sosmed asli perusahaan.
- Prediksi jenis pekerjaan yang tersedia.
- Reputasi "Black Company" atau ulasan buruk.

### 3. 📝 Email Generator Otomatis
Tidak perlu bingung menulis surat lamaran. Fitur ini membuatkan **Cover Letter** dalam Bahasa Jepang standar bisnis (`Keigo`) secara otomatis. Cukup isi data diri, copy, dan kirim.

### 4. 📚 Philia Guidebook
Panduan lengkap interaktif di dalam aplikasi yang membahas:
- Strategi mencari TSK "Kolam Kecil" (Persaingan rendah).
- Teknik OSINT (Open Source Intelligence) untuk riset perusahaan.
- Peta krisis tenaga kerja di Jepang.

---

## ⚖️ Lisensi & Legalitas

Proyek ini dilisensikan di bawah **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)**.

### Anda Diizinkan Untuk:
- **Berbagi**: Menyalin dan menyebarluaskan materi ini dalam bentuk atau format apapun.
- **Adaptasi**: Menggubah, mengubah, dan membuat turunan dari materi ini.

### Dengan Syarat:
1.  **Atribusi (BY)**: Anda harus mencantumkan nama **Philia Kensaku** & **Philia Space Community** sebagai pembuat asli.
2.  **Non-Komersial (NC)**: Anda **TIDAK BOLEH** menggunakan materi/data ini untuk tujuan komersial (Jual beli data, layanan berbayar, dll).
3.  **Berbagi Serupa (SA)**: Jika Anda mengubah atau membuat turunan dari materi ini, Anda harus menyebarluaskannya di bawah lisensi yang sama (Gratis & Terbuka).

📄 *Lihat file [LICENSE.txt](./LICENSE.txt) untuk teks hukum lengkap.*

---

## 🤝 Kredit & Penanggung Jawab

Aplikasi ini dikembangkan dan dikelola oleh:

*   **Komunitas**: Philia Space Community
*   **Operator AI & Lead Developer**: **Putera Perdana Gemilang Baang**
*   **Teknologi AI**: Google AI Studio (Gemini Models)
*   **Sumber Data**: Kementerian Kehakiman Jepang (Ministry of Justice - MOJ)

> *Saya, Putera Perdana Gemilang Baang, bertanggung jawab penuh atas operasional prompt AI dan kurasi sistem ini. Segala ketidakakuratan data adalah tanggung jawab operator dan bukan afiliasi pihak ketiga.*

---

## ⚠️ Disclaimer (Penafian)

Aplikasi ini adalah **Alat Bantu Riset (Research Tool)**.
- Philia Kensaku **BUKAN** agen penyalur (LPK/SO).
- Philia Kensaku **BUKAN** perwakilan pemerintah Indonesia maupun Jepang.
- Kami **TIDAK** memungut biaya sepeserpun.

Keputusan akhir untuk menandatangani kontrak kerja adalah tanggung jawab penuh pengguna. Selalu lakukan verifikasi mandiri (*Double Check*) sebelum mengambil keputusan karir.

---

### 🛠️ Cara Instalasi (Developer)

```bash
# 1. Clone Repo
git clone https://github.com/username/philia-kensaku.git

# 2. Install Dependencies
npm install

# 3. Jalankan Mode Dev
npm run dev
```

_Dibuat dengan ❤️ untuk Pejuang Devisa Indonesia._